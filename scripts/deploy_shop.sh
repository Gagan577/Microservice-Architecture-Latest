#!/bin/bash
# =============================================================================
# Deploy Shop Management Service - Server A (Port 8080)
# =============================================================================
# This script deploys the shop-management application on Server A
...
# Stock Service Configuration (Server B)
STOCK_SERVICE_HOST="${STOCK_SERVICE_HOST:-localhost}"
STOCK_SERVICE_PORT="${STOCK_SERVICE_PORT:-8081}"
...
stop_existing_process() {
    ...
    # Also check for any process on port 8080
...
start_application() {
    ...
    export DB_USERNAME
    export DB_PASSWORD
    export STOCK_SERVICE_HOST
    export STOCK_SERVICE_PORT
    ...
    nohup java ${JAVA_OPTS} \
        ...
        -DSTOCK_SERVICE_HOST=${STOCK_SERVICE_HOST} \
        -DSTOCK_SERVICE_PORT=${STOCK_SERVICE_PORT} \
        -DLOG_PATH=${LOG_DIR} \
...
health_check() {
    ...
    while [ ${RETRY_COUNT} -lt ${MAX_RETRIES} ]; do
        HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:${APP_PORT}/api/shop/health 2>/dev/null || echo "000")
...
main() {
    ...
    if health_check; then
        ...
        log "API URL: http://localhost:${APP_PORT}/api/shop"
        log "Swagger: http://localhost:${APP_PORT}/api/shop/swagger-ui.html"
        log "Logs: ${LOG_DIR}"
        log "=========================================="
    else
